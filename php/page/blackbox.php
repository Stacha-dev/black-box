...
if ($_POST['prjId']) {
  ...
  for ($i = 0; $i < $imgsCount; $i++) {
    ...
    $vyobrazeni .= '<li>'.($i+1).'. '.str_replace("/data/", "", $imgs[$i]).' <span class=\\"locate\\" post=\\"'.($i+1).'\\"></span></li>';
  }

  $vyobrazeni .= '<li post=\\"postMain\\">'.($i+1).'. HLAVNI FOTO <span class=\\"locate\\" post=\\"postMain\\"></span></li>';
